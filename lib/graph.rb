module Graph
  ...
  def graphArray(graph_hash = { })
    ...
    puts "============================"
    puts found_objects
    puts found_objects.class
    
    
    days = elapsed_time.to_i/60/60/24
    
    output_array = Array.new(days+1, 0)
    date_array = Array.new(days+1, 0)
    
    found_objects.each do |object|
        0.upto(days) do |date|           
            date_array[date] = Time.now.end_of_day - date.days if date_array[date] == 0                      
